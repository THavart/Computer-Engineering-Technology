...
Token malar_next_token(Buffer * sc_buf)
{
	...
	*/
	...
	while (1){ /* endless loop broken by token returns it will generate a warning */
		...
		switch (c){
		case 255: //end of file //change to 255
			t.code = SEOF_T;
			return t;
...
Token aa_func02(char lexeme[]){
	...
	unsigned int i;
	//printf("|%s|", lexeme);
	if ((t.attribute.kwt_idx = iskeyword(lexeme)) > -1){
		t.code = KW_T;
		return t;
	}
	/*Set AVID Token*/
...
/*****************************************************************************/
/* ERROR - Will not output necessary # if main error has the -1.
/*****************************************************************************/
/*
* Purpose:			SVID
* Author:			Taylor Havart-Labrecque
