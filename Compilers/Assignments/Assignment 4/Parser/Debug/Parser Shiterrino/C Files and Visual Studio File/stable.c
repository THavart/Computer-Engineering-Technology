	...
	if (offset == -1){
		...
		for (i = 0; i < strlen(lexeme); i++){
			...
			if (b_rflag(sym_table.plsBD)){
				r_flag = b_rflag(sym_table.plsBD);
			}
		}
		...
		if (r_flag) {
			offset = 0; /* The offset is used for the mark location. */

			/*
			* For every entry in the symbol table, update the memory location
			* pointed to by 'plex'.
			*/
			for (i = 0; i <= (unsigned int)sym_table.st_offset; ++i) {
				sym_table.pstvr[i].plex = b_setmark(sym_table.plsBD, (short)offset);
