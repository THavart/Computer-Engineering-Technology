<?php
global $wpdb;			
	$email_body_cilent = get_option( 'ep_fitness_order_client_email');
	$order_email_subject_client = get_option( 'ep_fitness_order_client_email_sub');			
	...
	$dis_amount =get_user_meta($userId, 'ep_fitness_discount',true);
	
	$package_amount_2=$ep_fitness_package_cost.' '.$api_currency;
	$discount_2= $dis_amount.' '.$api_currency;
	$total_g =($ep_fitness_package_cost - $dis_amount).' '.$api_currency;
	...
	// For Admin Email*****	
	$email_body_admin = get_option( 'ep_fitness_order_admin_email');
	$order_email_subject_admin = get_option( 'ep_fitness_order_admin_email_sub');			
