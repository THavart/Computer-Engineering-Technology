<?php
	...
	if(sizeof($membership_users)>0){
		...
		{	
			$user_id= $row->ID;
			$reminder_day = get_option( 'ep_fitness_reminder_day');
			...
			if( $reminder_day >= $days ){
					...
					if(strtotime($exprie_send_email_date) != strtotime($exp_date) || $exprie_send_email_date=='' ){
						// Start Email Action
							...
							$email_body = get_option( 'ep_fitness_reminder_email');
							$signup_email_subject = get_option( 'ep_fitness_reminder_email_subject');			
	...
	$sql="SELECT * FROM $wpdb->posts WHERE post_type IN ('".$post_type."' )   and post_status='publish' ";									
	...
	if(sizeof($total_post)>0){
		...
		foreach ( $all_post as $row )								
		{
			...
			$post_author_id=$row->post_author;	
				...
				$exp_date= get_user_meta($post_author_id, 'ep_fitness_exprie_date', true);
				if($exp_date!=''){
					$package_id=get_user_meta($post_author_id,'ep_fitness_package_id',true);
					$dir_hide= get_post_meta($package_id, 'ep_fitness_package_hide_exp', true);
					if($dir_hide=='yes'){
