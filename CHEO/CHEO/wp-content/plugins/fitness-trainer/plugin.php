  ...
  if (!class_exists('wp_ep_fitness')) {  	
			...
			private static $instance;
				...
				public function __construct() {
					...
					add_action('wp_ajax_ep_add_my_activities', array(&$this, 'ep_add_my_activities'));
					add_action('wp_ajax_nopriv_ep_add_my_activities', array(&$this, 'ep_add_my_activities'));	
					
					add_action('wp_ajax_ep_remove_my_activities', array(&$this, 'ep_remove_my_activities'));
					add_action('wp_ajax_nopriv_ep_remove_my_activities', array(&$this, 'ep_remove_my_activities'));	
					
					add_action('wp_ajax_ep_updata_userdata', array(&$this, 'ep_updata_userdata'));
					add_action('wp_ajax_nopriv_ep_updata_userdata', array(&$this, 'ep_updata_userdata'));	
				...
				public function ep_fitness_check_login(){
					...
					$dk_login = get_user_meta($user->ID, 'user_active', true);
						...
						if($dk_login == 0){						
							...
							if($iv_redirect!='defult'){
								if(empty($user_info->first_name) || empty($user_info->last_name)){
									$reg_page = get_permalink($iv_redirect).'/?profile=myaccount';
								}else{
									$reg_page = get_permalink( $iv_redirect).'/?profile=activities'; 
								}
				...
				*/
				...
				public function ep_add_my_activities(){ 
					$user_id = $_POST['user_id'];
					$post_id = $_POST['post_id'];
					//add_user_meta( $user_id, 'my_activities', $post_id);
					$get_post = array_unique(get_user_meta($user_id, 'my_activities', false));
				...
				public function ep_remove_my_activities(){
					$user_id = $_POST['user_id'];
					$post_id = $_POST['post_id'];
					delete_user_meta( $user_id, 'my_activities', $post_id );
					echo json_encode(array("status" => 'true'));
					
					exit(0);
				} 
				public function ep_updata_userdata(){
					$user_id = $_POST['user_id'];
					$first_name = $_POST['first_name'];
					$last_name = $_POST['last_name'];
					$user_info = wp_update_user( array('ID' => $user_id, 'first_name' => $first_name, 'last_name'=> $last_name ) );
					if ( is_wp_error( $user_info ) ) {
						echo json_encode(array("update" => 'flase'));
					} else {
