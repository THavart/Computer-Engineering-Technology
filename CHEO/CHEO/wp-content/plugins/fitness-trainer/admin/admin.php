...
if (!class_exists('wp_ep_fitness_Admin')) {
	class wp_ep_fitness_Admin {
		...
		public function __construct() {
			...
			add_action('wp_ajax_ep_fitness_update_form_registration', array(&$this, 'ep_fitness_update_form_registration'));
			...
			add_action('wp_ajax_ep_fitness_update_profile_template', array(&$this, 'ep_fitness_update_profile_template'));
			add_action('wp_ajax_ep_fitness_update_signup_template', array(&$this, 'ep_fitness_update_signup_template'));
			...
			add_action('wp_ajax_ep_fitness_update_post_setting', array(&$this, 'ep_fitness_update_post_setting'));
			add_action('wp_ajax_ep_fitness_update_protected_setting', array(&$this, 'ep_fitness_update_protected_setting'));
			add_action('wp_ajax_iv_update_protected_page_setting', array(&$this, 'iv_update_protected_page_setting'));
			
			add_action('wp_ajax_ep_fitness_archive_template', array(&$this, 'ep_fitness_archive_template'));
			
			add_action('wp_ajax_ep_fitness_update_map_marker', array(&$this, 'ep_fitness_update_map_marker'));			
			add_action('wp_ajax_ep_fitness_update_cate_image', array(&$this, 'ep_fitness_update_cate_image'));			
			add_action('wp_ajax_ep_fitness_update_profile_public_template', array(&$this, 'ep_fitness_update_profile_public_template'));
			add_action('wp_ajax_ep_fitness_update_user_directory', array(&$this, 'ep_fitness_update_user_directory'));			
			add_action('wp_ajax_ep_fitness_update_user_settings', array(&$this, 'ep_fitness_update_user_settings')); 			
			add_action('wp_ajax_ep_fitness_update_price_table_template', array(&$this, 'ep_fitness_update_price_table_template'));			
			...
			add_action('wp_ajax_ep_fitness_email_client_template_change', array(&$this, 'ep_fitness_email_client_template_change'));
			add_action('wp_ajax_ep_fitness_save_package_table', array(&$this, 'ep_fitness_save_package_table'));
			...
			add_action('wp_ajax_ep_fitness_update_doctor_fields', array(&$this, 'ep_fitness_update_doctor_fields'));
			...
			add_action('wp_ajax_iv_update_review_fields', array(&$this, 'iv_update_review_fields'));
...
// Hook into the 'init' action
		...
		public function ep_fitness_payment_post_type() {
			...
			public function ep_fitness_update_map_marker(){					
			...
			public function ep_fitness_update_cate_image(){
			...
			public function custom_iv_payment_column( $column, $post_id ) {
				...
				public function admin_menu() {
					add_menu_page('WP ep_fitness', 'Fitness Trainer', 'manage_options', 'wp-ep_fitness', array(&$this, 'menu_hook'));
					...
					self::$pages['wp-ep_fitness-profile-form'] = add_submenu_page('wp-ep_fitness', 'WP ep_fitness Profile', 'My Account', 'manage_options', 'wp-ep_fitness-profile-form', array(&$this, 'menu_hook'));		
					...
					self::$pages['wp-ep_fitness-user-directory'] = add_submenu_page('wp-ep_fitness', 'WP ep_fitness Directory', 'User Directory', 'manage_options', 'wp-ep_fitness-user-directory', array(&$this, 'menu_hook'));	
					...
					self::$pages['wp-ep_fitness-post_type'] = add_submenu_page('wp-ep_fitness', 'WP ep_fitness post_type', 'Post Type', 'manage_options', 'wp-ep_fitness-post_type', array(&$this, 'menu_hook'));
					...
					self::$pages['wp-ep_fitness-report'] = add_submenu_page('wp-ep_fitness', 'WP ep_fitness Report', 'Report', 'manage_options', 'wp-ep_fitness-report', array(&$this, 'menu_hook'));						
				...
				public function menu_hook() {
					...
					switch ($screen->id) {
						...
						case self::$pages['wp-ep_fitness-profile-form']:
						...
						break;
						case self::$pages['wp-ep_fitness-report']:
							
							include ('pages/report.php');							
						break;
						case self::$pages['wp-ep_fitness-user-directory']:							
							include ('pages/user_directory_wizard.php');
						...
						case self::$pages['wp-ep_fitness-post_type']:							
				...
				public function load_scripts() {
					...
					if (in_array($screen->id, array_values(self::$pages))) {
						wp_enqueue_script('ep_fitness-script-3', wp_ep_fitness_URLPATH . 'admin/files/js/jquery-ui.min.js');
						//wp_enqueue_script('ep_fitness-script-3', wp_ep_fitness_URLPATH . 'admin/files/js/jquery-ui.js');
						wp_enqueue_script('ep_fitness-script-4', wp_ep_fitness_URLPATH . 'admin/files/js/bootstrap.min.js');
						//wp_enqueue_script('ep_fitness-script-4', wp_ep_fitness_URLPATH . 'admin/files/js/jquery.ui.widget.js');
						wp_enqueue_script('ep_fitness-script-5', wp_ep_fitness_URLPATH . 'admin/files/js/jquery.ui.touch-punch.min.js');	
						wp_enqueue_script('ep_fitness-script-1', wp_ep_fitness_URLPATH . 'admin/files/js/handlebars.min.js');
					}
				...
				public function load_styles() {
					$screen = get_current_screen();
					if (in_array($screen->id, array_values(self::$pages))) {
						wp_enqueue_style('wp-ep_fitness-style-3', wp_ep_fitness_URLPATH . 'admin/files/css/jquery-ui.css');	
					}
				...
				public function ep_fitness_save_package() {
					...
					$last_post_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_type = 'ep_fitness_pack' ORDER BY `ID` DESC ");
					...
					$post_content= $form_data['package_feature']; 
						...
						update_post_meta($newpost_id, 'ep_fitness_package_cost', $form_data['package_cost']);
						update_post_meta($newpost_id, 'ep_fitness_package_initial_expire_interval', $form_data['package_initial_expire_interval']);							
						update_post_meta($newpost_id, 'ep_fitness_package_initial_expire_type', $form_data['package_initial_expire_type']);
						
						update_post_meta($newpost_id, 'iv_directories_display_order', $form_data['package_order']);
						
						if(isset($form_data['package_recurring'])){
							update_post_meta($newpost_id, 'ep_fitness_package_recurring', $form_data['package_recurring']);
						}else{
							update_post_meta($newpost_id, 'ep_fitness_package_recurring', '');
						}
						
						
						//update_post_meta($newpost_id, '1_time_subscription_fee', $form_data['package_1_time_subscription_fee']);
						update_post_meta($newpost_id, 'ep_fitness_package_recurring_cost_initial', $form_data['package_recurring_cost_initial']);
						update_post_meta($newpost_id, 'ep_fitness_package_recurring_cycle_count', $form_data['package_recurring_cycle_count']);
						update_post_meta($newpost_id, 'ep_fitness_package_recurring_cycle_type', $form_data['package_recurring_cycle_type']);
						update_post_meta($newpost_id, 'ep_fitness_package_recurring_cycle_limit', $form_data['package_recurring_cycle_limit']);
						
						if(isset($form_data['package_enable_trial_period'])){
							update_post_meta($newpost_id, 'ep_fitness_package_enable_trial_period', $form_data['package_enable_trial_period']);
						}else{
							update_post_meta($newpost_id, 'ep_fitness_package_enable_trial_period', 'no');
						}
						update_post_meta($newpost_id, 'ep_fitness_package_trial_amount', $form_data['package_trial_amount']);
						update_post_meta($newpost_id, 'ep_fitness_package_trial_period_interval', $form_data['package_trial_period_interval']);
						update_post_meta($newpost_id, 'ep_fitness_package_recurring_trial_type', $form_data['package_recurring_trial_type']);
						...
						if(isset($form_data['package_recurring'])){
							...
							if($iv_gateway=='stripe'){
								include(wp_ep_fitness_DIR . '/admin/files/lib/Stripe.php');
				...
				public function iv_update_dir_setting(){
						...
						update_option('_dir_load_listing_all',$form_data['dir_load_listing_all']);
						update_option('_dir_load_doctor_all',$form_data['dir_load_doctor_all']);
						update_option('_dir_comments_show',$form_data['dir_comments_show']);
						update_option('_dir_slider_auto',$form_data['dir_slider_auto']);
						update_option('_dir_tag_show',$form_data['dir_tag_show']);
						
						
						update_option('_dir_search_keyword',$form_data['dir_search_keyword']);
						update_option('_dir_search_city',$form_data['dir_search_city']);
						update_option('_dir_search_country',$form_data['dir_search_country']);
						update_option('_dir_search_tag',$form_data['dir_search_tag']);
						update_option('_dir_search_category',$form_data['dir_search_category']);
						update_option('_dir_search_location',$form_data['dir_search_location']);
						update_option('_dir_f_search_radius',$form_data['dir_f_search_radius']);
						update_option('_dir_search_zipcode',$form_data['dir_search_zipcode']);
						update_option('_dir_search_radius',$form_data['dir_search_radius']);
						
						update_option('_dir_top_filter',$form_data['dir_top_filter']);
						update_option('_dir_color',$form_data['dir_color']);
						update_option('_dir_map_zoom',$form_data['dir_map_zoom']);
				...
				public function ep_fitness_save_package_table(){
				
					$package_html_content =	$_REQUEST['ep_fitness_package_template'];
					$package_html_content = str_replace('container', 'container-fluid', $package_html_content);
					
					update_option('ep_fitness_package_template', htmlentities(stripslashes($package_html_content)));
					
					//update_option('ep_fitness_package_template', stripslashes(wp_filter_post_kses(addslashes($_POST['ep_fitness_package_template']))) );
					//update_option('ep_fitness_package_template', $_POST['ep_fitness_package_template'] );
					echo json_encode(array('code' => 'success'));
					exit(0);
				   
				}
				public function iv_update_review_fields(){
					...
					update_option('iv_cpt-1_fields_review', $opt_array );
				...
				public function ep_fitness_update_profile_fields(){
					...
					// remove menu******
					 if(isset($form_data['listinghome'])){
						update_option( '_ep_fitness_menu_listinghome' ,$form_data['listinghome']); 
					 }else{
						update_option( '_ep_fitness_menu_listinghome' ,'no') ; 
					 }
					 if(isset($form_data['mylevel'])){
						update_option( '_ep_fitness_mylevel' ,$form_data['mylevel']); 
					 }else{
						update_option( '_ep_fitness_mylevel' ,'no') ; 
					 }
					 
					 if(isset($form_data['menusetting'])){
						update_option( '_ep_fitness_menusetting' ,$form_data['menusetting']); 
					 }else{
					 ...
					 if(isset($form_data['menurecords'])){
						update_option( '_ep_fitness_menurecords' ,$form_data['menurecords']); 
					 }else{
						update_option( '_ep_fitness_menurecords' ,'no') ; 
					 }							 
					 
					  if(isset($form_data['menuadd-record'])){
						update_option( '_ep_fitness_menuadd-record' ,$form_data['menuadd-record']); 
					 }else{
				...
				public function  ep_fitness_update_post_type(){
					...
					{	
						if($form_data['posttype_name'][$i]!="" AND $form_data['posttype_label'][$i]!=""){
							$opt_array[$form_data['posttype_name'][$i]]=$form_data['posttype_label'][$i];
						}
					}	
				...
				public function ep_fitness_update_doctor_fields(){
					...
					update_option('ep_fitness_fields_doctor', $opt_array );
			...
			public function ep_fitness_update_paypal_settings() {
					...
					global $wpdb;			
						...
						update_post_meta($newpost_id, 'ep_fitness_paypal_mode', $form_data['paypal_mode']);
						update_post_meta($newpost_id, 'ep_fitness_paypal_username', $form_data['paypal_username']);
						update_post_meta($newpost_id, 'ep_fitness_paypal_api_password', $form_data['paypal_api_password']);
						update_post_meta($newpost_id, 'ep_fitness_paypal_api_signature', $form_data['paypal_api_signature']);
						update_post_meta($newpost_id, 'ep_fitness_paypal_api_currency', $form_data['paypal_api_currency']);
						
					    update_option('_ep_fitness_api_currency', $form_data['paypal_api_currency'] );
				...
				public function ep_fitness_update_stripe_settings() {
					...
					global $wpdb;			
						...
						update_post_meta($newpost_id, 'ep_fitness_stripe_mode', $form_data['stripe_mode']);
						
						update_post_meta($newpost_id, 'ep_fitness_stripe_live_secret_key', $form_data['secret_key']);						
						update_post_meta($newpost_id, 'ep_fitness_stripe_live_publishable_key', $form_data['publishable_key']);
						
						update_post_meta($newpost_id, 'ep_fitness_stripe_secret_test', $form_data['secret_key_test']);						
						update_post_meta($newpost_id, 'ep_fitness_stripe_publishable_test', $form_data['stripe_publishable_test']);						
						update_post_meta($newpost_id, 'ep_fitness_stripe_api_currency', $form_data['stripe_api_currency']);
						
					    update_option('_ep_fitness_api_currency', $form_data['stripe_api_currency'] );
		...
		public function ep_fitness_update_coupon() {
			...
			public function	ep_fitness_update_profile_template(){
				...
				if(isset($_POST['profile-st'])){
					...
					update_option('ep_fitness_profile-template', $profile_style); 
				}
				echo json_encode(array('code' => 'Update successfully'));
				exit(0);
			}	
			
			public function	ep_fitness_archive_template(){
			...
			public function ep_fitness_update_user_directory(){
				$dir_style='';				
				if(isset($_POST['profile-st'])){
					$dir_style=$_POST['profile-st'];
					
					update_option('ep_fitness_user_directory', $dir_style); 
				}
				echo json_encode(array('code' => 'Update successfully'));
				exit(0);
			
			}
			public function ep_fitness_update_profile_public_template(){
				$profile_style='';
				
				if(isset($_POST['profile-st'])){
					$profile_style=$_POST['profile-st'];
					
					update_option('ep_fitness_profile-public', $profile_style); 
				}
				echo json_encode(array('code' => 'Update successfully'));
				exit(0);
			}
			public function ep_fitness_update_signup_template(){
				...
				if(isset($_POST['signup-st'])){
					...
					update_option('ep_fitness_signup-template', $signup_style); 
				}
				echo json_encode(array('code' => 'Update successfully'));
				exit(0);
			}
		public function	ep_fitness_update_price_table_template(){
				...
				if(isset($_POST['price-tab-style'])){
					...
					update_option('ep_fitness_price-table', $profile_style); 
				}
...
public function ep_fitness_update_form_registration() {
	...
	$ep_fitness_email_template_type="";
	if (isset($_POST['ep_fitness_email_template_type'])) {
		$ep_fitness_email_template_type = $_POST['ep_fitness_email_template_type'];
	}
	...
	if (isset($_POST['form_content']) AND $_POST['form_content'] <> "") {
		...
		$form_content = str_replace('submit_ep_fitness', $submit_id, $form_content);
		$form_content = str_replace('z-index', '', $form_content);
		$form_content = str_replace('well2', '', $form_content);
		
		$ep_fitness_width_old = get_post_meta($form_id, 'ep_fitness_width', true);
		$ep_fitness_width = '';
		
		//$form_content = str_replace('change_width_iv', $ep_fitness_width, $form_content);
	...
	if (isset($_POST['script_content']) AND $_POST['script_content'] <> "") {
		...
		//$form_script_content = $form_script_content + '<script type="text/javascript" src="' . wp_ep_fitness_URLPATH . 'admin/files/js/bootstrapValidator.min.js" ></script>';
		...
		$form_script_content = str_replace('submit_ep_fitness', $submit_button, $form_script_content);
	...
	if (strpos($form_content,'contact_captcha1') !== false) {
		...
		if (strpos($form_content,'contact_date') !== false) {
				$script_ui='<link href="' . wp_ep_fitness_URLPATH . 'admin/files/css/jquery-ui.css" rel="stylesheet" media="screen">
				<script type="text/javascript" src="' . wp_ep_fitness_URLPATH . 'admin/files/js/jquery-ui.min.js" ></script>';
		...
		$form_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name = 'ep_fitness_account_form'");
		...
		update_post_meta($newpost_id, 'ep_fitness_content', $form_content_main);
...
public function ep_fitness_update_form_payment() {
	...
	if (isset($_POST['form_content']) AND $_POST['form_content'] <> "") {
		...
		$ep_fitness_width_old = get_post_meta($form_id, 'ep_fitness_width', true);
		$ep_fitness_width = $_POST['ep_fitness_width'];
	...
	if (isset($_POST['script_content']) AND $_POST['script_content'] <> "") {
		...
		$form_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name = 'ep_fitness_payment_form'");
...
public function ep_fitness_update_account_setting(){
		...
		update_option('ep_fitness_post_approved', $post_approved );
...
public function iv_update_protected_page_setting(){
		...
		update_option('_ep_fitness_active_visibility_page', $active_visibility );
...
public function ep_fitness_update_protected_setting(){
		parse_str($_POST['form_data'], $form_data);
		...
		update_option('_ep_fitness_active_visibility', $active_visibility );
...
public function ep_fitness_update_post_setting(){
		parse_str($_POST['form_data'], $form_data);				
		...
		update_option('_ep_fitness_profile_post', $post_type );
...
public function ep_fitness_update_email_setting(){
		...
		update_option( 'ep_fitness_signup_email_subject',$form_data['ep_fitness_signup_email_subject']);
		update_option( 'ep_fitness_signup_email',$form_data['signup_email_template']);
		
		update_option( 'ep_fitness_forget_email_subject',$form_data['forget_email_subject']);
		...
		update_option('ep_fitness_order_client_email_sub', $form_data['ep_fitness_order_email_subject']); 
		update_option('ep_fitness_order_client_email', $form_data['order_client_email_template']); 
		update_option('ep_fitness_order_admin_email_sub', $form_data['ep_fitness_order_admin_email_subject']);
		update_option('ep_fitness_order_admin_email', $form_data['order_admin_email_template']); 			
						
		update_option( 'ep_fitness_reminder_email_subject',$form_data['ep_fitness_reminder_email_subject']);
		update_option( 'ep_fitness_reminder_email',$form_data['reminder_email_template']);		 
		update_option('ep_fitness_reminder_day', $form_data['ep_fitness_reminder_day']); 
		
		update_option( 'ep_fitness_denial_email_subject',$form_data['ep_fitness_denial_email_subject']);
		update_option( 'ep_fitness_denial_email',$form_data['denial_email_template']); 
		
		update_option( 'ep_fitness_denial_active_email_subject',$form_data['ep_fitness_denial_active_email_subject']);
		update_option( 'ep_fitness_active_denial_email',$form_data['denial_activer_email_template']); 
							
		update_option( 'ep_fitness_contact_email_subject',$form_data['contact_email_subject']);
		...
		///////
		update_option( 'ep_fitness_refund_email_subject',$form_data['refund_email_subject']);
		...
		update_option( 'ep_fitness_deal_email_subject',$form_data['deal_email_subject']);
		update_option( 'ep_fitness_deal_email',$form_data['ep_fitness_deal_email']);		
...
public function ep_fitness_update_mailchamp_setting (){
		...
		update_option('ep_fitness_mailchimp_api_key', $form_data['ep_fitness_mailchimp_api_key']); 
		update_option('ep_fitness_mailchimp_confirmation', $form_data['ep_fitness_mailchimp_confirmation']); 
		if(isset($form_data['ep_fitness_mailchimp_list'])){
				update_option('ep_fitness_mailchimp_list', $form_data['ep_fitness_mailchimp_list']); 
		}
...
public function ep_fitness_email_client_template_change() {	
	$ep_fitness_template = $_POST['ep_fitness_template'];
		$settings = array('textarea_rows' => 20,);
		$content_admin = get_option('ep_fitness_client_email_'.$ep_fitness_template );
		echo $content_admin;
		exit(0);
}	
...
public function ep_fitness_update_user_settings(){
	...
	update_user_meta($user_id, 'ep_fitness_payment_status', $form_data['payment_status']);	
	update_user_meta($user_id, 'iv_user_content_setting', $form_data['content_setting']);	
...
public function ep_fitness_settings_save() {
	...
	//update_option('ep_fitness_admin_email_subject', $ep_fitness_admin_email_subject);
	//update_option('ep_fitness_auto_email_subject', $ep_fitness_auto_email_subject);
...
public function get_all_settings() {
	...
	$settings = array();
	$seriazlized_settings = array('ep_fitness_selected_attributes');
	foreach ($all_options as $name => $value) {
		if (stristr($name, 'ep_fitness_')) {
