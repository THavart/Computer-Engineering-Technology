...
.html-active .switch-html, .tmce-active .switch-tmce {
	...
	.wp-switch-editor {
		...
		<h3  class=""><?php _e('Pages Restriction By Roles','epfitness'); ?>  <small></small>	
		</h3>
		...
		<div id="update_message"> </div>	
			<form class="form-horizontal" role="form"  name='protected_page_settings' id='protected_page_settings'>											
				<?php
					...
					?>
					 <div class="row">
						<label  class="col-md-3  pull-left"> <?php _e('Page Visibility Module :','epfitness'); ?> </label>
						<div class="col-md-3">
							<label>												
							<input type="radio" name="active_visibility_page" id="active_visibility_page" value='yes' <?php echo $active_check_y; ?> > <?php _e('Show Page By Role Access','epfitness'); ?>
							</label>	
						</div>
						<div class="col-md-3">	
							<label>											
							<input type="radio"  name="active_visibility_page" id="active_visibility_page" value='no' <?php echo $active_check_n; ?> ><?php _e('Show All','epfitness'); ?> 
							</label>
						</div>												
					 </div>
					
					<div class=" col-md-12  bs-callout bs-callout-info">							
						<?php _e('Select which pages are available for each user role.	','epfitness'); ?>										
					</div>
					...
					<div class="row ">
						...
						<table class="table table-bordered table-hover table-responsive">												  
						  <thead>
						   ...
						   <tfoot>
							<tr>
							  <th> <?php _e('Check/Uncheck','epfitness'); ?> </th>
							  <?php
							  global $wp_roles;
							  //$roles = $wp_roles->get_names();
							  //foreach($roles as $role) {
					...
					<div class="col-md-8">
						...
						<button type="button" onclick="return  iv_update_protected_settings_page();" class="btn btn-success"><?php _e('Update','epfitness'); ?></button>
					</div>
				</div>
