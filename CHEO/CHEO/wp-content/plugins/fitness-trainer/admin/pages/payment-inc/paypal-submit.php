...
if( isset($_REQUEST['iv-submit-listing']) && isset($_REQUEST['payment_gateway']) && $_REQUEST['iv-submit-listing']=='register' ){	
				...
				//create user here******	
					...
					if(isset($_POST['iv_member_email'])){
						...
						$userdata['user_login']=$_POST['iv_member_email'];
					}					
					...
					if($userdata['user_login']!='' and $userdata['user_email']!='' and $userdata['user_pass']!='' ){
							...
							if ( !$user_id and email_exists($userdata['user_email']) == false ) {							
								...
								/* Send mail user */
								// $to_user = $userdata['user_email'];
								// $ep_fitness_order_client_email_sub = get_option( 'ep_fitness_order_client_email_sub');	
								// $ep_fitness_order_client_email = get_option( 'ep_fitness_order_client_email');	
																
								// wp_mail( $to_user, $ep_fitness_order_client_email_sub, $ep_fitness_order_client_email, $headers );
								
								/* Send mail admin */
								$to_addmin = get_option( 'admin_email' );
								$ep_fitness_order_admin_email_subject = get_option( 'ep_fitness_order_admin_email_sub');
								$ep_fitness_order_admin_email = get_option( 'ep_fitness_order_admin_email');
								$text = str_replace('%email%', $to_user, $ep_fitness_order_admin_email);
								wp_mail( $to_addmin, $ep_fitness_order_admin_email_subject, $text, $headers );
			...
			if($package_cost >0){
					...
					$paymentDescription = $ep_fitness_package_cost.' '.$currencyCode.' for '.$package_name .' at '. get_bloginfo();
					...
					if($ep_fitness_package_recurring=='on'){
							...
							$params = $urlParams + $recurring;
									...
									// CouponStart ******************************										
											...
											$dis_amount=0;
											$package_amount=$ep_fitness_package_cost;
					...
					// Coupon End *****************************
									$ep_fitness_package_cost = $ep_fitness_package_cost- $dis_amount;
							
									$orderParams = array(
									'PAYMENTREQUEST_0_AMT' => $ep_fitness_package_cost,
									'PAYMENTREQUEST_0_SHIPPINGAMT' => '0',
									'PAYMENTREQUEST_0_CURRENCYCODE' => $currencyCode,
									'PAYMENTREQUEST_0_ITEMAMT' => $ep_fitness_package_cost
									);
									$itemParams = array(
									'L_PAYMENTREQUEST_0_NAME0' => $paymentName,
									'L_PAYMENTREQUEST_0_DESC0' => $paymentDescription,
									'L_PAYMENTREQUEST_0_AMT0' =>  $ep_fitness_package_cost,
					...
					if(is_array($response) && $response['ACK'] == 'Success') {
								...
								$peried=get_post_meta( $package_id,'ep_fitness_package_recurring_cycle_type',true);
								$cycle_count=get_post_meta( $package_id,'ep_fitness_package_recurring_cycle_count',true);
									...
									update_user_meta($userId, 'ep_fitness_payment_gateway','paypal-express'); 
									update_user_meta($userId, 'iv_paypal_recurring_profile_init_amt',get_post_meta( $package_id,'ep_fitness_package_cost',true));
						...
						die('User has created. But PayPal API keys are not workimg : '.$response['L_LONGMESSAGE0']);
	...
	if($tokenRow){
		...
		if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
			...
			if($ep_fitness_package_recurring=='on'){
				...
				$arb_limit	= get_post_meta( $package_id,'ep_fitness_package_recurring_cycle_limit',true); 
				if($arb_limit!=''){
				...
				if( $trial_enable=='yes' ){					
					...
					$trial_amount= get_post_meta( $package_id,'ep_fitness_package_trial_amount',true); 
					$trial_period_interval= get_post_meta( $package_id,'ep_fitness_package_trial_period_interval',true); 
					$trial_type = get_post_meta( $package_id,'ep_fitness_package_recurring_trial_type',true);
				...
				if( is_array($recurringPayment) && $recurringPayment['ACK'] == 'Success' ) {
					...
					update_user_meta($userId, 'ep_fitness_exprie_date',$begins); 
					update_user_meta($userId, 'ep_fitness_package_id',$package_id);
				...
				if( is_array($singlePayment) && $singlePayment['ACK'] == 'Success') {
					...
					//New code for expire date
							 ...
							 update_user_meta($userId, 'ep_fitness_exprie_date',$exp_d); 
							 update_user_meta($userId, 'ep_fitness_package_id',$package_id);
			...
			if($package_cost >0){
					...
					if(is_array($response) && $response['ACK'] == 'Success') {
								...
								$cycle_count=get_post_meta( $package_id,'ep_fitness_package_recurring_cycle_count',true);
									...
									update_user_meta($userId, 'iv_paypal_recurring_profile_period',$peried);	
									update_user_meta($userId, 'iv_paypal_recurring_cycle_count',$cycle_count);
									update_user_meta($userId, 'ep_fitness_discount','');
...
if( isset($_REQUEST['iv-submit-balance']) &&  $_REQUEST['iv-submit-balance']=='addbalance' ){
	...
	if($amount >0){
			...
			if(is_array($response) && $response['ACK'] == 'Success') {
					...
					update_user_meta($userId, '_ep_fitness_balance_submit','');
