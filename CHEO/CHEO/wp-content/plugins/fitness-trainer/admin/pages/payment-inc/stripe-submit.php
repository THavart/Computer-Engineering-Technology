			...
			if($package_cost >0){
					...
					if($ep_fitness_package_recurring=='on'){
							...
							if( $trial_enable=='yes' ){
									...
									$trial_amount=0;//get_post_meta($package_id, 'ep_fitness_package_trial_amount', true);									
									if($trial_amount>0){
							...
							try {
								...
								if(isset($customer->subscriptions->data[0]->id)){
									update_user_meta($userId, 'ep_fitness_stripe_subscrip_id', $customer->subscriptions->data[0]->id);
								}
