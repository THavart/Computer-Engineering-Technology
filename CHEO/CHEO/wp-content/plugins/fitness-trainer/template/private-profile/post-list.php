...
wp_enqueue_style('ep_fitness-style-toast', wp_ep_fitness_URLPATH . 'assets/cube/css/jquery.toast.css');
...
if(isset($_GET['door']) || isset($_GET['winter']) || isset($_GET['music']) || isset($_GET['group']) || isset($_GET['age-group']) || isset($_GET['fitness'])){
	$args = array(
		'post_type' => $profile, // enter your custom post type
		'post_status' => 'publish',
		'post__not_in' => $get_my_activities,
...
// if((isset($_GET['door']) && $_GET['door'] == '0') || (isset($_GET['winter']) && $_GET['winter'] == '0') || (isset($_GET['music']) && $_GET['winter'] == '0') || (isset($_GET['group']) && $_GET['group'] =='0') || (isset($_GET['age-group']) && $_GET['age-group'] == '0') || (isset($_GET['fitness']) && $_GET['fitness'] == '0')) {
// 	$args = array(
// 		'post_type' => $profile, // enter your custom post type
// 		'post_status' => 'publish',
// 		'post__not_in' => $get_my_activities,
...
if(isset($_GET['winter']) &&  ($_GET['winter'] != '0')){
	 $args['tax_query'][] = array(
		'relation' => 'AND',
		array(
			'taxonomy' => 'winter',
...
if(isset($_GET['age-group']) && ($_GET['age-group']) != '0'){
	  $args['tax_query'][] = array(
		'relation' => 'AND',
		array(
			'taxonomy' => 'age-group',
			'field' => 'slug',
			'terms' => $_GET['age-group'],
		),
		array(
			'taxonomy' => 'restrictions',
			'field'    => 'term_id',
...
if(isset($_GET['fitness']) && ($_GET['fitness']) != '0'){
	  $args['tax_query'][] = array(	
		'relation' => 'AND',
		array(
			'taxonomy' => 'fitness',
...
?>
	...
	<div class="portlet light">
		...
		<div class="clearfix">
			<?php	
				// var_dump($_GET['data_term']);
				// var_dump($_GET['term_name']);
				$args = array(
							    'orderby'    => 'title',
							    'order'      => 'ASC',
								'hide_empty' => false
				...
				printSelectFilter($terms, 'door', 'Indoor / Outdoor','door_select', $option, $_GET['door']);
				...
				printSelectFilter($terms, 'winter', 'Winter / Non-Winter','winter_select', $option, $_GET['winter']);
				...
				printSelectFilter($terms, 'music', 'Music / Creativity','music_select', $option, $_GET['music']);
				...
				printSelectFilter($terms, 'group', 'Group / Alone','group_select', $option, $_GET['group']);
				...
				printSelectFilter($terms, 'age-group', 'Age Group','age_group_select', $option, $_GET['age-group']);
				...
				printSelectFilter($terms, 'fitness', 'Fitness','fitness_select', $option, $_GET['fitness']);
			?>      
		</div>
...
<?php 
	function printSelectFilter($terms, $data_term, $label, $select_class, $option_check, $get_door){
		$profile=(isset($_REQUEST['profile'])?$_REQUEST['profile']:$f_cpt);
		global $wp;
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ){
			...
			echo '<div class="select-filter-item">';
				...
				echo '<select class="'.$select_class.'">';
					echo '<option key-value="0" data-link="'.home_url($wp->request).'/?profile='.$profile.'" data-term="'.$data_term.'" data-name="0">Select</option>';
					foreach ( $terms as $term ) {
						if(!in_array($term->term_id, $option_check))
						?>
							<!--echo '<option "'.$select.'" data-term="'.$data_term.'" data-name="'.$term->name.'" data-link="'.home_url($wp->request).'/?profile='.$profile.'&data_term='.$data_term.'&term_name='.$term->name.'" value="'.$term->term_id.'">'.$term->name.'</option>';-->					    
							<option key-value="1" <?php if( $get_door == $term->name ){ echo 'selected'; } ?> data-term="<?php echo $data_term; ?>" data-name="<?php echo $term->name ?>" data-link="<?php echo $current_url.'&'.$data_term.'='.$term->name; ?>" value="<?php echo $term->term_id; ?>"><?php echo $term->name; ?></option>
						<?php
					}
				echo '</select>';
			echo '</div>';
...
wp_enqueue_script('ep_fitness-ar-script-toast', wp_ep_fitness_URLPATH . 'assets/cube/js/jquery.toast.js');						
?>
...
jQuery( document ).ready(function() {
		...
		if (confirm("Are you sure?")) {
			jQuery.ajax({
				...
				success : function( data ) {
					...
					var table = jQuery('#my_activities_table').DataTable();
					table.row( find ).remove().draw();
				}
			});
		}
	...
	jQuery('.select-filter-item select').on('change', function() {
		...
		var term_name  = jQuery('option:selected', this).attr('data-name');
		var user_id = jQuery(".user_id").val();
		var key_value = jQuery('option:selected', this).attr('key-value');
		var location = jQuery('option:selected', this).attr('data-link');
		var url = window.location.href;
		var updata_url = updateQueryStringParameter(url, term, term_name);
		if(key_value == 0){
			window.location = location;
		}else{
			console.log(key_value);
			window.location = updata_url;
		}
...
#profile-account2 a.paginate_button.current{
	padding: 5px 10px;
    line-height: 1.3;
}
#profile-account2 a.paginate_button.previous,
#profile-account2 a.paginate_button.next,#all_activities_table_info, #my_activities_table_info{
	font-size:14px;
}
</style>
